...
export const useUploadAboutPage = () => {
  ...
  const uploadAboutPage = useCallback(async (title, body, subBody, images) => {
    ...
    try {
      const checkResponse = await fetch("/api/aboutPage/check", {
        method: "GET",
        credentials: "include",
      });
      if (checkResponse.ok) {
        ...
        if (exists && id) {
          await fetch(`/api/aboutPage/delete/${id}`, {
            method: "DELETE",
            credentials: "include",
          });
        }
      ...
      const response = await fetch("/api/aboutPage/create", {
        method: "POST",
        body: formData,
        credentials: "include",
      });
      ...
      if (error.message === "Authentication required") {
        ...
        if (refreshSuccess) {
          const retryResponse = await fetch("/api/aboutPage/create", {
            method: "POST",
            body: formData,
            credentials: "include",
          });

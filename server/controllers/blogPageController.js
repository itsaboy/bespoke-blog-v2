...
export const createBlogPagePost = async (c) => {
  ...
  const imageUrls = await Promise.all(
    imageFiles.map((file) =>
      uploadImageToS3(process.env.BUCKET_NAME, file, title, "BlogPage")
    )
  );
  ...
  try {
    ...
    await blogPage.save();
    return c.json({ message: "Content created successfully", blogPage });
  } catch (error) {
...
export const getBlogPagePosts = async (c) => {
  try {
    const blogPage = await BlogPage.find().sort({ createdAt: -1 });
    return c.json(blogPage);
  } catch (error) {
...
export const deleteBlogPagePost = async (c) => {
...
export const checkBlogPagePostExists = async (c) => {
  try {
